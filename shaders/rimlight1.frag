...
vec4 applyHSBCEffect(vec4 color) {
	//Brightness
	color.rgb += ((brightness) / 255.0);
	
	//Hue
	color.rgb = applyHueRotate(color.rgb, hue);
...
vec3 getDropShadow(vec4 col, float curThreshold, bool useMask) {
	...
	// add the dropshadow colorbased on the amount, strength, and intensity
	return (min(dropColor.rgb * ((1.0 - (dropShadowAmount * str))*intensity), 1.) * pow(col.a, 16.)); // uhm. yea. sure
}
...
void main()
{
	...
	vec4 unpremultipliedColor = vec4(col.a > 0.0 ? col.rgb / col.a : col.rgb, col.a);

	vec4 outColor = applyHSBCEffect(unpremultipliedColor);
	
	gl_FragColor = vec4((outColor.rgb * col.a + getDropShadow(col, thr, useMask)) * openfl_Alphav, col.a * openfl_Alphav);
	
	if (openfl_HasColorTransform || hasColorTransform) gl_FragColor = (gl_FragColor * openfl_ColorMultiplierv + openfl_ColorOffsetv);
}